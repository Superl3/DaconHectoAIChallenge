...
def get_spm(input, target, conf, model):
    imgsize = (conf.get('crop_size'), conf.get('crop_size'))
    bs = input.size(0)
    with torch.no_grad():
        output, fms, _ = model(input)
        if 'inception' in conf.get('netname', ''):
...
def snapmix(input, target, conf, model=None):
    ...
    beta = float(conf.get('beta', 5))
    if r < float(conf.get('prob', 0.5)):
...
def as_cutmix(input, target, conf, model=None):
    ...
    if r < float(conf.get('prob', 0.5)):
        ...
        lam = 1 - ((bbx2 - bbx1) * (bby2 - bby1) / (input.size()[-1] * input.size()[-2]))
        lam_a = torch.full_like(lam_a, lam)
        lam_b = torch.full_like(lam_b, 1 - lam)
    return input, target, target_b, lam_a.cuda(), lam_b.cuda()
...
def cutout(input, target, conf=None, model=None):
    ...
    lam_a = lam
    lam_b = 1 - lam
    prob = float(conf.get('prob', 0.5)) if conf is not None else 0.5
    if r < prob:
        bs = input.size(0)
        lam_val = 0.75
        bbx1, bby1, bbx2, bby2 = utils.rand_bbox(input.size(), lam_val)
